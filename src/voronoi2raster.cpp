...
Rcpp::List tiles2contours(const Eigen::VectorXd &tiles,
  ...
  for ( int iter = 0, pos = 0 ; iter < niters ; iter++ ) {
  ...
  return Rcpp::List::create(Rcpp::Named("zrates") = raster,
                            Rcpp::Named("pr_gt0") = pr_gt0,
                            Rcpp::Named("pr_lt0") = pr_lt0,
                            Rcpp::Named("niters") = niters);
}
